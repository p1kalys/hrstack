...
const Navbar = () => {
    ...
    return (
        <div className='flex justify-between items-center py-5 mx-4 sm:mx-20 xl:mx-32'>
            ...
            {token && <button onClick={() => navigate('/profile')} className='flex items-center gap-2 rounded-full text-xs sm:text-sm cursor-pointer bg-primary font-medium text-white px-5 sm:px-10 py-1.5 sm:py-2.5'>{'Dashboard'}<img src={assets.arrow} className='w-3' alt='arrow' /></button>}
        </div>
    )
}
